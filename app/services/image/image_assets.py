...
def _get_upload_dir() -> Path:
    ...
    # 否则，尝试相对于 worker 目录的上级（项目根）
    # 本文件位置: worker/app/services/image/image_assets.py
    worker_dir = Path(__file__).parent.parent.parent.parent  # 向上4级到 worker/
    project_root = worker_dir.parent  # 向上1级到项目根
    backend_uploads = project_root / "backend" / settings.UPLOAD_DIR
    
    if backend_uploads.exists():
        print(f"[image_assets] 使用后备上传目录: {backend_uploads}")
